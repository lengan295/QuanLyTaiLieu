...
namespace QuanLyTaiLieu
{
    public partial class frmCapNhat : Form
    {
        ...
        private void UpdateInfo()
        {
            ...
            txtTieude.Text = tl.TieuDe;
            txtNam.Text = tl.Nam + "";
            txtDOI.Text = tl.DOI;
            txtTomtat.Text = tl.TomTat;
            txtTep.Text = tl.File;
            txtLink.Text = tl.URL;
            switch (tl.LoaiTaiLieu.Trim().ToLower())
            {
                case "article":
                    cbbLoaiTL.SelectedIndex = 0;
                    BaiBao bb = dbcon.getBaiBao(tl);
                    txtTapchi.Text = bb.TapChi;
                    txtTrang.Text = bb.Trang;
                    if (bb.Volume != 0)
                        txtVolume.Text = bb.Volume + "";
                    if (bb.Issue != 0)
                        txtIssue.Text = bb.Issue + "";
                    break;
                case "inproceedings":
                    cbbLoaiTL.SelectedIndex = 1;
                    Proceedings pr = dbcon.getProceeding(tl);
                    txtTenhoinghi.Text = pr.TenHoiNghi;
                    txtThanhpho.Text = pr.ThanhPho;
                    break;
                case "book":
                    cbbLoaiTL.SelectedIndex = 2;
                    Sach sh = dbcon.getSach(tl);
                    txtNXB.Text = sh.NhaXB;
                    txtTaiban.Text = sh.TaiBan;
                    txtThanhpho.Text = sh.ThanhPho;
                    break;
                case "misc":
                    cbbLoaiTL.SelectedIndex = 3;
                    TrangWeb web = dbcon.getTrangWeb(tl);
                    txttochuc.Text = web.ToChuc;
                    if (web.Ngay != 0)
                        txtngay.Text = web.Ngay + "";
                    if (web.Thang != 0)
                        txtthang.Text = web.Thang + "";
                    txtngaytruycap.Text = web.NgayTruyCap.ToString("dd/MM/yyyy");
                    break;
            }

            List<DanhMuc> listdm = dbcon.getDanhMucByTaiLieu(tl);
            foreach (DanhMuc dm in listdm)
            {
                for (int i=0; i< checklistbox.Items.Count; i++)
                {
                    ListBoxItem item = (ListBoxItem)checklistbox.Items[i];
                    if (((DanhMuc)item.Tag).MaDM == dm.MaDM)
